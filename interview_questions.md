...
4. **Analysis Dispatch**: The client triggers `POST /OpenAIAws/OpenAISummary` passing the `fileUrl`.
...
6. **AI Analysis**: For images, OpenAI GPT-4o Vision processes the pre-signed URL. For PDFs, `PdfPig` extracts the text stream and chunks it before calling OpenAI.
...
### Q2: Why did you use S3 Pre-Signed URLs instead of streaming binary files directly through your API to OpenAI?
**Answer**:
...
### Q3: How did you implement caching, and what problem does it solve?
**Answer**:
* **Problem**: Invoking multimodal LLM endpoints (like GPT-4o Vision) repeatedly for the same uploaded assets introduces latency (1–3 seconds per request) and racks up expensive API token fees.
* **Solution**: In `FileAnalysisService`, before sending any request to OpenAI, we query the `FileAnalysisResult` table joined on the unique file URL.
* **Result**: Repeat analysis requests return in under 10ms with zero OpenAI token cost.

---

### Q4: How do you handle large PDF documents that exceed LLM context windows?
**Answer**:
...
### Q5: How is authentication and user authorization handled?
**Answer**:
...
### Q6: Why did you choose the Repository and Unit of Work patterns over using `DbContext` directly in controllers?
**Answer**:
...
### Q7: What happens if an unsupported or corrupted file is uploaded?
**Answer**:
...
### Q8: What are the bottlenecks in this architecture, and how would you scale it?
**Answer**:
* **Current Bottleneck**: Synchronous HTTP calls to OpenAI can take 2–5 seconds during peak traffic. If 1,000 users upload simultaneously, HTTP request threads could get exhausted.
...
2. **Vector Embeddings & Semantic Search**: Index parsed PDF and text summaries using OpenAI `text-embedding-3-small` and store them in a vector database (e.g. pgvector or Azure AI Search) for natural language semantic search across all uploaded files.
3. **Automated CI/CD**: Add GitHub Actions pipelines for automated linting, security scanning, and unit testing with `xUnit` and `moto`/LocalStack.
